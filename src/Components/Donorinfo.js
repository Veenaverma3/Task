    ...
    <div className="flex flex-col lg:flex-row items-start justify-evenly w-full p-6 space-y-6 lg:space-y-0 lg:space-x-6">
      ...
      <div className="bg-white shadow-md rounded-lg w-full max-w-xs p-6">
        {[  
          ...
          { title: "Add Attorney", active: false },
          { title: "Add Preferences", active: false },
          { title: "Final Check: LPA Details", active: false },
        ].map((step, index) => (
          <div key={index}>
            <div className="flex items-center space-x-4">
              <div className={`h-6 w-6 rounded-full ${step.active ? "bg-teal-400" : "bg-gray-400"}`}></div>
              <h2 className="text-lg font-semibold">{step.title}</h2>
            </div>
            {index !== 3 && (
              <div className={`h-10 ml-2.5 w-1 my-3 ${step.active ? "bg-teal-400" : "bg-gray-400"}`}></div>
            )}
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col w-full max-w-2xl space-y-6">
        ...
        <div className="p-6 bg-white shadow rounded-lg">
          <div className="mb-6">
            ...
            <h1 className="text-3xl font-serif">Enter Donor's Information</h1>
          </div>
          ...
          {/* Address Section */}
          <div className="mt-6">
            <h1 className="text-2xl font-serif">Enter Address Manually</h1>
            <div className="grid grid-cols-1 gap-4 mt-4">
              ...
              <label className="text-gray-700 font-medium">Address Line 3</label>
              <input type="text" className="p-2 border rounded w-full" />
            </div>
    ...
    Add Donor
        ...
        <div className="bg-white shadow-md rounded-lg p-6">
          <div className="flex justify-between items-center">
            <p className="text-gray-600 font-semibold">The Donor</p>
